<div class="bg-white dark:bg-gray-800 w-full p-4 rounded shadow-lg">
	<h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight mb-4">
		{{ __('holiday.add_holiday') }}
	</h2>
	<div class="gap-2 flex flex-col">

		{{-- Nazionale? / Ricorrente? --}}
		<div class="w-full flex flex-col sm:flex-row sm:gap-9 gap-2">
			<div class="w-full">
				<x-checkbox
					label="{{ \Str::ucfirst(__('holiday.is_recurring')) }}"
					wire:model.defer="isNational"
				/>
			</div>
			<div class="w-full">
				<x-checkbox
					label="{{ \Str::ucfirst(__('holiday.is_national')) }}"
					wire:model.defer="isRecurring"
				/>
			</div>
		</div>
		...
		<div class="grid grid-cols-3 gap-4 mt-4">
			@foreach($worksites as $key => $worksite)
				<div class="flex items-center">
					...
					<label for="worksite-{{ $worksite->id }}" class="ml-3 text-sm font-medium drak:text-white">
						{{ $worksite->cod }}
					</label>
				</div>
			@endforeach
